...
| **SB2 WIFI** | Sensorbox wifi *(Only available if MAINS METER is set to **Sensorbox**, requires Sensorbox v2, Sensorbox plus is not compatible)* | **Disabled** No wifi connection.<br/><br/>**SetupWifi** Once the Sensorbox2 is connected to your Wifi, you can find it's IP address on the SmartEVSE's SB2 WIFI menu option; connect with your phone to that access point, goto http://192.168.4.1/ and configure your Wifi password.<br/><br/>**Enabled** Connect to the previosly setup Wifi.
| **MAINSADR** | Set the Modbus address for the kWh meter *(Only available if MAINS METER is set to a kWh meter)* | 
| **SOL BOOST** | Exceed MAINS MAX capacity using solar surplus *(Only available in Smart mode and if MAINS METER Sensorbox is in use)*. Useful in 3 phases installations with 1 phase cable and solar panels. | Enabled, disabled
