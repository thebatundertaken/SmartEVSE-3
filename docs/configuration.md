...
| **MAINS METER** | Type of MAINS meter *(only avaiable in Smart or Solar mode)* | **Disabled** No meter connected, only Normal mode possible.<br/><br/>**Sensorbox** Sensorbox will send measurement data to the SmartEVSE.<br/><br/>**Phoenix C / Finder / Eastron / ...** a Modbus kWh meter is used. *Note that Eastron is for single phase Eastron meters.*
| **GRID** | 3 phase installation over 3 or 4 wires *(Only available if MAINS METER is set to **Sensorbox**)* |
| **MAINSADR** | Set the Modbus address for the kWh meter *(Only available if MAINS METER is set to a kWh meter)* | 
| **SOLAR BOOST** | Exceed MAINS MAX capacity using solar surplus *(Only available if MAINS METER is in use)*. Useful in 3 phases installations with 1 phase cable and solar panels. | 
...
| **CIRCUIT** | If PWR SHARE set to &lt;Disabled&gt;: Only appears when an EV METER is configured, in Smart or Solar mode. Set the max current the EVSE circuit can handle (load balancing). Not obeyed in Normal mode.<br/>If PWR SHARE set to &lt;Master&gt;: Set the max current the EVSE circuit can handle (load balancing). Obeyed in all modes! | 10-200A
